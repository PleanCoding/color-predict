# Color picker for RGB range system
...
def pick_color(event,x,y,flags,param):
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        R_upper = check_boundaries(pixelRGB[0], 10, 1, 1)
        R_lower = check_boundaries(pixelRGB[0], 10, 1, 0)
        G_upper = check_boundaries(pixelRGB[1], 10, 1, 1)
        G_lower = check_boundaries(pixelRGB[1], 10, 1, 0)
        B_upper = check_boundaries(pixelRGB[2], 10, 1, 1)
        B_lower = check_boundaries(pixelRGB[2], 10, 1, 0)

        upper =  np.array([R_upper, G_upper, B_upper])
        lower =  np.array([R_lower, G_lower, B_lower])
        ptxt = f"point({x},{y}),RGB({pixelRGB[0]},{pixelRGB[1]},{pixelRGB[2]}),HSV({pixelHSV[0]},{pixelHSV[1]},{pixelHSV[2]}),lower_RGB{lower},upper_RGB{upper}"
        print(ptxt)

        #A MONOCHROME MASK FOR GETTING A BETTER VISION OVER THE COLORS 
        image_mask = cv2.inRange(image_rgb,lower,upper)
